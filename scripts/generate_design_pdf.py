...
def build_design_pdf() -> Path:
    ...
    story += [table([
        ...
        ["语音合成", "不使用预录备份", "qwen3-tts-flash-2025-11-27 / Momo / Chinese"],
    ...
    story += [p("评测集固定为 60 题，用于同时验证“答对”和“该拒时拒答”。本地配置和百炼正式配置分别留存评测记录，避免把本地索引结果冒充生产结果。"),
              ...
              status_box("本地与生产已验证", "2026-09-13：30 项单测通过，包含扫描 PDF 页码绑定与百炼向量接口临时 403 重试；22 个来源、347 个知识块的生产向量固定 60 题为 Top-6 命中 40/40、拒答 20/20；正式 API 20 轮为 20/20，Momo 完整问答语音链路返回 HTTP 200。", GREEN),
              p("验收记录与待办", H2)]
