...
- 22 个知识来源：2 份原创角色设定、10 篇学术参考资料、4 份官方资料摘编和 6 份审核网页摘编。
- SQLite FTS5 词法检索与向量检索融合，单一来源最多返回 2 个知识块。
- 问题主题门控、提示词攻击检测、证据相关度阈值、引用 ID 白名单和第二次语义核验。
- 只有完成校验的回答才能取得一次性语音令牌；未校验内容无法调用播报接口。
- React、Three.js、`@pixiv/three-vrm` 三维舞台，支持 VRMA 待机/思考状态切换、眨眼、视线跟随和音频振幅口型。
...
- 30 项后端单元/接口测试全部通过，包含扫描 PDF 页码绑定、原文件哈希与百炼向量接口临时 403 重试覆盖；前端生产构建通过。
...
- 百炼正式 API 20 轮复测为 20/20 符合预期；其中 2 轮实际调用 `/api/speech`均返回 HTTP 200。现用非实时 HTTP TTS `qwen3-tts-flash-2025-11-27` 的 `Momo` 中文音色；本次单独短句实测为 5.36 秒、24 kHz 单声道 WAV，新增风筝魏问题的完整问答—引用—语音链路也已返回 HTTP 200。
- 上述时长是生成音频长度，不是网络延迟承诺。VRM 技术验收已经通过，但书面授权、署名方式和联系方式隐私确认仍未完成。
